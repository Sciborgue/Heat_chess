...
class Sidebar:
    ...
    def draw_status_panel(self, win, x, y, width, height):
        ...
        win.blit(font.render(f"W: {data['white_belt']} ({data['white_threats']})", True, (220, 220, 220)), (col_xs[2], offset_y + spacing))
        win.blit(font.render(f"B: {data['black_belt']} ({data['black_threats']})", True, (220, 220, 220)), (col_xs[2], offset_y + spacing * 2))
        win.blit(font.render(f"Center T:{data['tight_center']} L:{data['large_center']}", True, (200, 200, 200)), (col_xs[2], offset_y + spacing * 3))

    def draw_status_panel21(self, win, x, y, width, height):
        font = pygame.font.SysFont(None, 22)
        pygame.draw.rect(win, (40, 40, 40), (x, y, width, height))
        ...
        # --- Control Influence ---
        win.blit(font.render("Control Influence", True, (180, 180, 180)), (x + 10, offset_y))
        offset_y += 20
        win.blit(font.render(f"White: {data['white_control']}", True, (255, 255, 255)), (x + 10, offset_y))
        win.blit(font.render(f"Black: {data['black_control']}", True, (255, 255, 255)), (x + 130, offset_y))
        win.blit(font.render(f"Balance: {data['global_influence_score']}", True, (255, 255, 255)), (x + 250, offset_y))
        offset_y += 30

        # --- Square Possession ---
        win.blit(font.render("Square Possession", True, (180, 180, 180)), (x + 10, offset_y))
        offset_y += 20
        win.blit(font.render(f"White: {data['white_squares']}", True, (255, 255, 255)), (x + 10, offset_y))
        win.blit(font.render(f"Black: {data['black_squares']}", True, (255, 255, 255)), (x + 130, offset_y))
        win.blit(font.render(f"Neutral: {data['total_squares']}", True, (255, 255, 255)), (x + 250, offset_y))
        offset_y += 30

        # --- King Belt Threats ---
        win.blit(font.render("King Belt Threats", True, (180, 180, 180)), (x + 10, offset_y))
        offset_y += 20
        win.blit(
            font.render(
                f"White: {data['white_belt_score']}  ({data['white_threat_count']} threats)", 
                True, (255, 255, 255)), 
            (x + 10, offset_y)
        )
        offset_y += 20
        win.blit(
            font.render(
                f"Black: {data['black_belt_score']}  ({data['black_threat_count']} threats)", 
                True, (255, 255, 255)), 
            (x + 10, offset_y)
        )
        offset_y += 30
        ...
        # --- Center Control ---
        win.blit(font.render("Center Control", True, (180, 180, 180)), (x + 10, offset_y))
        offset_y += 20
        win.blit(font.render(f"Tight: {data['tight_center_score']}", True, (255, 255, 255)), (x + 10, offset_y))
        win.blit(font.render(f"Large: {data['large_center_score']}", True, (255, 255, 255)), (x + 130, offset_y))
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            for method, buttons in self.method_weight_buttons.items():
                if buttons["decrease"].collidepoint(event.pos):
                    self.adjust_method_weight(method, -0.05)
                    return
