...
class Game:
    ...
    def update_game_state(self, win, tree_width, tree_depth):
        ...
        #Update the board
        self.board.update_board_state(self.turn,tree_width, tree_depth)
        #update the sidebar
        self.sidebar.update_sidebar(self.board.predictions, win,self.turn, self.board.white_control, self.board.black_control, self.board.global_influence_score)
        self.selected_piece = None
...
def main():
    ...
    while run:
        ...
        selected_method = game.sidebar.methods[game.sidebar.selected_method]
        if selected_method == 'Heat':
            game.board.draw_heatmap(win)
        elif selected_method == 'Belt':
            game.board.draw_belt(win)
        elif selected_method == 'Integrity':
            game.board.draw_integrity(win)
        # Draw the sidebar
